  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-16 sm:px-6 sm:py-24">
      <header className="max-w-3xl">
        <ShieldCheckIcon className="text-ink-04 mb-4 size-8" strokeWidth={1.75} />
        <h1 className="text-3xl leading-tight font-semibold tracking-hero sm:text-5xl">
          Security at Onirix
        </h1>
        <p className="text-ink-03 mt-5 text-lg leading-7">
      ...
      <section className="border-border/60 mt-20 border-t pt-12">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          ...
          <div className="bg-card flex flex-col gap-4 rounded-xl border p-6">
            ...
            <Button
              ...
              render={<a href="mailto:contact@logestalabs.com" />}
            >
        ...
        <div className="mt-10">
          <Button render={<Link href="/login?mode=signup" />}>Get started</Button>
        </div>
      </section>
    </div>
  );
...
function Summary({ title, body }: { title: string; body: string }) {
  return (
    <div className="bg-card rounded-xl border p-6">
      ...
      <p className="text-ink-03 mt-2 text-sm leading-6">{body}</p>
    </div>
  );
}
...
function Block({
  ...
  return (
    <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)]">
      <div>
        <Icon className="text-ink-04 mb-3 size-6" strokeWidth={1.75} />
        <h2 className="text-xl font-semibold tracking-heading">{title}</h2>
        <p className="text-ink-03 mt-2 text-sm leading-6">{lead}</p>
      </div>
      <dl className="divide-border/60 flex flex-col divide-y">{children}</dl>
    </section>
  );
}
