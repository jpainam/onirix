...
import { Container, Eyebrow, Heading, Lead, Section, SectionIntro } from "@/components/section";
  ...
  return (
    <>
      ...
      <Container className="pt-10 pb-14 sm:pt-16 sm:pb-20">
        <div className="motion-safe:animate-rise-in flex max-w-3xl flex-col items-start">
          <Eyebrow>Security</Eyebrow>
          <Heading as="h1" className="mt-4 text-3xl sm:text-4xl">
            What the product does with your documents, stated precisely.
          </Heading>
          <Lead className="mt-5">
            The controls that exist in the product today. The page is silent
            about anything that does not.
          </Lead>
        </div>
      </Container>

      {/* The three that matter most, on sand */}
      <section aria-label="Summary" className="bg-wash-sand border-t">
        <Container>
      ...
      {/* The controls, on paper */}
      <div className="border-t">
        <Container className="grid gap-12 py-14 sm:py-20 lg:grid-cols-[13rem_minmax(0,1fr)] lg:gap-20">
          <nav
      ...
      {/* Certifications, on stone */}
      <Section id="certifications" labelledBy="certifications-heading" className="bg-tint-01">
        <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
          <SectionIntro
            eyebrow="Certifications"
            title="No audit claimed. Every control checkable."
            titleId="certifications-heading"
            lead="Onirix has not completed a third-party audit such as SOC 2 or ISO 27001, and this page does not claim one. Each control above can be checked against the source."
          />
          <div className="flex flex-col items-start gap-4 border-t pt-6 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-16">
            <h3 className="text-base font-medium">Security contact</h3>
            <p className="text-ink-03 text-sm leading-6">
              Questionnaires, data handling questions, and vulnerability reports
              all go to the same address. Please include enough detail for us to
              reproduce a finding.
            </p>
            <CtaLink href="mailto:contact@logestalabs.com" variant="secondary">
      ...
      {/* Closing, on ink */}
      <Section labelledBy="security-start-heading" className="dark bg-tint-01 text-foreground">
        <div className="flex flex-col items-start gap-8">
          <SectionIntro
            eyebrow="Self-hosting"
            title="Keep the data under controls you already operate."
            titleId="security-start-heading"
            lead="Self-hosting keeps data in the region and on the infrastructure your organization runs, which is usually the first thing a data protection review asks about."
          />
          <div className="flex flex-wrap gap-3">
            <CtaLink href={SIGN_UP_URL}>Create a workspace</CtaLink>
...
function Summary({ title, body }: { title: string; body: string }) {
  return (
    <div className="flex flex-col gap-2 border-b py-8 last:border-b-0 md:border-r md:border-b-0 md:px-7 md:first:pl-0 md:last:border-r-0 md:last:pr-0">
      <h2 className="text-base font-medium">{title}</h2>
      <p className="text-ink-03 text-sm leading-6 text-pretty">{body}</p>
    </div>
  );
}
...
function Block({
  ...
  return (
    <section
      id={id}
      className="grid scroll-mt-24 gap-6 md:grid-cols-[minmax(0,1fr)_minmax(0,1.6fr)] md:gap-10"
    >
      <div className="flex flex-col items-start gap-3">
        <span className="bg-tint-02 text-ink-04 flex size-9 items-center justify-center rounded-xl">
          <Icon className="size-4" strokeWidth={1.75} aria-hidden />
        </span>
        <h2 className="text-xl font-medium tracking-tight">{title}</h2>
        <p className="text-ink-03 text-sm leading-6 text-pretty">{lead}</p>
      </div>
      <dl className="flex flex-col divide-y border-y">{children}</dl>
    </section>
  );
}
...
function Item({ title, body }: { title: string; body: string }) {
  return (
    <div className="py-5">
      ...
      <dd className="text-ink-03 mt-1 text-sm leading-6 text-pretty">{body}</dd>
    </div>
  );
}
